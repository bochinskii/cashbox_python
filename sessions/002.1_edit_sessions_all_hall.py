...
# Проверка существования таблицы с ценами - "prices".
try:
    cursor.execute("SELECT id, price FROM prices")
    get_prices_check = cursor.fetchall()
except sqlite3.OperationalError:
    print('Не возможно создать сеанс т.к. нет ни одной цены. Создайте сперва таблицу с ценами.')
    exit()
...
"3" - название шоу запланированное на данную дату;\n\
"4" - дату проведения сеанса;\n\
"5" - цену места;\n\
"6" - цену для последовательности мест;\n\
Введите нужное значение: '))
    ...
    if choice == 1:
        while True:
            ...
            status_new = '''
            UPDATE {session} SET status_id = :status_id WHERE date = :date AND hall_id = :hall_id
            '''
            cursor.execute(status_new.format(session=f'\'sessions-hall\''),
                           {'date': date, 'status_id': status_id, 'hall_id': hall_id})
            db.commit()
...
Нажмите клавишу "Enter", если "да" или введите "n" и нажмите "Enter", если хотите выйти: ')
    ...
    if choice == 2:
        while True:
            while True:
                try:
                    hall_row_id_start = int(
                        input('Введите номер ряда, с которого будет происходить изменение статуса: '))
                    hall_seat_id_start = int(
                        input('Введите номер места, с которого будет происходить изменение статуса: '))
                    hall_row_id_end = int(input('Введите номер ряда, по который будет происходить изменение статуса: '))
                    hall_seat_id_end = int(
                        input('Введите номер места, по которое будет происходить изменение статуса: '))
            ...
            while s <= hall_id_end:
                ...
                UPDATE {session} SET status_id = :status_id WHERE date = :date AND hall_id = :hall_id
                '''
                cursor.execute(status_many.format(session=f'\'sessions-hall\''),
...
Нажмите клавишу "Enter", если "да" или введите "n" и нажмите "Enter", если хотите выйти: ')
    ...
    if choice == 3:
        ...
        '''
        cursor.execute(show_new.format(session=f'\'sessions-hall\''),
    ...
    if choice == 4:
        ...
        if get_session_hall:
            ...
            test_warning_1 = input(
                'Нажмите "ENTER", чтобы посмотреть на этот сеанс или "n" и "ENTER", чтобы не выйти: ')
            if test_warning_1 != 'n':
        ...
        cursor.execute("UPDATE 'sessions-hall' SET date = :date_new WHERE date = :date",
    ...
    if choice == 5:
        while True:
            ...
            # Проверка на то, что цена - целое положительное число и существует в таблице статусов
            while True:
                price = input('Ведите цену по-умолчанию для всех мест данного сеанса: ')
                if price:
                    try:
            ...
            price_new = '''
            UPDATE {session} SET price_id = :price_id WHERE date = :date AND hall_id = :hall_id
            '''
            cursor.execute(price_new.format(session=f'\'sessions-hall\''),
                           {'date': date, 'price_id': price_id, 'hall_id': hall_id})
            db.commit()

            test_choice_5 = input('Хотите еще изменить статус ряда и места? \
Нажмите клавишу "Enter", если "да" или введите "n" и нажмите "Enter", если хотите выйти: ')
            if test_choice_5 == 'n':
                break

    # Изменяем цену последовательности мест
    if choice == 6:
        while True:
            while True:
                try:
                    hall_row_id_start = int(input('Введите номер ряда, с которого будет происходить изменение цены: '))
                    hall_seat_id_start = int(
                        input('Введите номер места, с которого будет происходить изменение цены: '))
                    hall_row_id_end = int(input('Введите номер ряда, по который будет происходить изменение цены: '))
                    hall_seat_id_end = int(input('Введите номер места, по которое будет происходить изменение цены: '))
            ...
            while True:
                ...
                if price:
                    try:
                        ...
                        cursor.execute('''SELECT id FROM prices WHERE price = :price''', {'price': price})
                        get_price_id = cursor.fetchall()
                        price_id_many = int(get_price_id[0]['id'])
                        break
                    except IndexError:
                        print('Вы ввели не существующую цену.')
            ...
            # Заполняем цену выбранных мест
            p = hall_id_start
            while p <= hall_id_end:
                price_many = '''
                UPDATE {session} SET price_id = :price_id WHERE date = :date AND hall_id = :hall_id
                '''
                cursor.execute(price_many.format(session=f'\'sessions-hall\''),
                               {'date': date, 'price_id': price_id_many, 'hall_id': p})
                db.commit()
                p += 1

            test_choice_6 = input('Хотите еще изменить цену мест?\n\
Нажмите клавишу "Enter", если "да" или введите "n" и нажмите "Enter", если хотите выйти: ')
            if test_choice_6 == 'n':
                break
