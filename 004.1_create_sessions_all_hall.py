...
# Создаем таблицу сеанса для Зала без цен
cursor.execute('''
...
# Создаем таблицу сеанса для зала с ценами, если ее нет (чтобы не было ошибки)
cursor.execute('''
...
while True:
    ...
    if price:
        try:
            ...
            test = input('Хотите использовать значение по-умолчанию - "0" руб.? \n\
Нажмите клавишу "Enter", если "да" или введите "n" и нажмите "Enter", если хотите выйти: ')
            if test != 'n':
                price_id = 1
                break
...
# Делаем проверку на то, что таблица с залом без цен заполнена
if not get_hall:
    print('Зал без цен не заполнен. Сперва заполните зал создайте зал.')
    exit()
...
try:
    for h in get_hall:
        cursor.execute('''
        INSERT INTO 'sessions-hall' (date, shows_id, hall_id, status_id, price_id) VALUES (strftime('%Y-%m-%d %H:%M',:date), :show, :hall, :status,\
        :price)''', {'date': date, 'show': show_id, 'hall': h['id'], 'status': status_id, 'price': price_id})
        db.commit()
...
if test == 'n':
    ...
    cursor.execute('''
    SELECT date, shows.name, categories.category, hall."row", hall.seat, status.status, prices.price\
    FROM 'sessions-hall'
    INNER JOIN shows ON shows_id = shows.id
    INNER JOIN hall ON hall_id = hall.id
    INNER JOIN status ON status_id = status.id
